# workOrder 个人代办清单 — 实现计划

## 1. 概述

本文档描述 workOrder MVP 的分阶段实现计划。技术栈为 **Vaadin Flow + Spring Boot 3 + SQLite**，交付形态为 **Windows 轻量桌面应用**。

**预计阶段**：4 个阶段，可串行完成。

---

## 2. 阶段总览

| 阶段 | 名称 | 目标 | 产出 |
|------|------|------|------|
| P1 | 项目脚手架 | 可运行的空壳应用 | Maven 工程 + SQLite 连通 |
| P2 | 领域层与数据层 | 核心业务逻辑就绪 | Entity / Repository / Service |
| P3 | Vaadin UI | 完整 MVP 交互 | 列表、编辑、筛选、拖拽、时间线 |
| P4 | 桌面打包 | 可分发桌面应用 | Windows exe + 启动器 |

```mermaid
flowchart LR
    P1[阶段1 脚手架] --> P2[阶段2 领域层]
    P2 --> P3[阶段3 UI]
    P3 --> P4[阶段4 打包]
```

---

## 3. 阶段一：项目脚手架（P1）

### 3.1 目标

创建 Maven 工程，集成 Spring Boot + Vaadin + SQLite，应用可启动并显示空白页面。

### 3.2 任务清单

| 序号 | 任务 | 细节 |
|------|------|------|
| P1-1 | 初始化 Maven 项目 | `groupId`: `com.workorder`，`artifactId`: `workorder` |
| P1-2 | 添加依赖 | `spring-boot-starter`、`vaadin-spring-boot-starter`、`spring-boot-starter-data-jpa`、`sqlite-jdbc`、sqlite Hibernate 方言、**flyway-core** |
| P1-3 | 配置 SQLite | `application.properties` 指向 `%USERPROFILE%/.workOrder/data/workorder.db` |
| P1-3b | 配置 Flyway | `spring.flyway.enabled=true`；**禁用** `ddl-auto=update`（开发可用 validate）；首版脚本 `V1__init_schema.sql` |
| P1-4 | 创建 Application 入口 | `WorkOrderApplication.java`，`@SpringBootApplication` |
| P1-5 | 创建空白 MainLayout | Vaadin `AppLayout` + 占位首页 |
| P1-6 | 验证启动 | `mvn spring-boot:run`，浏览器访问 localhost 可见页面 |

### 3.3 验收

- [ ] `mvn spring-boot:run` 无报错启动
- [ ] SQLite 数据库文件自动创建
- [ ] Vaadin 页面正常渲染

---

## 4. 阶段二：领域层与数据层（P2）

### 4.1 目标

实现数据模型、持久化、业务 Service，可通过单元测试或临时接口验证 CRUD。

### 4.2 数据模型

#### WorkOrder 实体

```java
@Entity
@Table(name = "work_order")
public class WorkOrder {
    @Id @GeneratedValue
    private Long id;
    private String title;              // 必填
    private String description;        // 可选
    @Enumerated(EnumType.STRING)
    private WorkOrderStatus status;    // 默认 NOT_STARTED
    private Integer priority;            // 排序序号，越小越靠前
    private String waitingFor;         // 等待对象，可选
    private String waitingReason;      // 等待原因，可选
    private LocalDateTime dueDate;     // 计划完成时间，可选
    private LocalDateTime createdAt;
    private LocalDateTime updatedAt;
}
```

#### WorkOrderStatus 枚举

```java
public enum WorkOrderStatus {
    NOT_STARTED,    // 未处置
    IN_PROGRESS,    // 已处置（进行中）
    WAITING_REPLY,  // 待回复
    COMPLETED       // 已完成
}
```

#### ProgressLog 实体

```java
@Entity
@Table(name = "progress_log")
public class ProgressLog {
    @Id @GeneratedValue
    private Long id;
    private Long workOrderId;
    private String content;
    private LocalDateTime createdAt;
}
```

### 4.3 任务清单

| 序号 | 任务 | 细节 |
|------|------|------|
| P2-1 | 创建枚举 WorkOrderStatus | 含中文 displayName 方法供 UI 使用 |
| P2-2 | 创建 WorkOrder 实体 | JPA 注解 + `@PrePersist` / `@PreUpdate` 自动维护时间 |
| P2-3 | 创建 ProgressLog 实体 | 外键关联 workOrderId |
| P2-4 | 创建 Repository 接口 | `WorkOrderRepository`、`ProgressLogRepository` |
| P2-5 | 实现 WorkOrderService | CRUD、按 priority 排序查询、按 status 筛选、批量更新 priority |
| P2-6 | 实现 ProgressLogService | 追加记录、按 workOrderId 查询（倒序） |
| P2-7 | 编写 Service 单元测试 | 覆盖创建、更新状态、排序、筛选、追加过程 |

### 4.4 Service 核心方法

```java
// WorkOrderService
WorkOrder create(WorkOrder workOrder);
WorkOrder update(Long id, WorkOrder workOrder);
void delete(Long id);
List<WorkOrder> findAll(Sort sort);
List<WorkOrder> findByStatuses(List<WorkOrderStatus> statuses, boolean includeCompleted);
void updatePriorities(List<Long> orderedIds);  // 拖拽后批量更新 priority
boolean isOverdue(WorkOrder workOrder);          // dueDate 过期且未完成

// ProgressLogService
ProgressLog addLog(Long workOrderId, String content);
List<ProgressLog> findByWorkOrderId(Long workOrderId);
```

### 4.5 验收

- [ ] 单元测试全部通过
- [ ] 可通过 Service 完成 CRUD、筛选、排序、过程记录

---

## 5. 阶段三：Vaadin UI（P3）

### 5.1 目标

实现 MVP 全部用户交互：列表、新建、编辑、删除、拖拽排序、状态筛选、过程时间线、过期高亮。

### 5.2 页面结构

```
MainLayout (AppLayout)
└── WorkOrderListView (默认路由 @Route "")
    ├── 顶部工具栏
    │   ├── [新建] 按钮
    │   ├── 状态筛选 MultiSelectComboBox
    │   └── [显示已完成] Checkbox
    ├── Grid<WorkOrder> 列表
    │   ├── 列：标题、状态、计划完成时间、最后更新
    │   ├── 行拖拽排序
    │   └── 过期行高亮（CSS class: overdue-row）
    └── WorkOrderDetailDialog（点击行打开）
        ├── 标题 TextField
        ├── 描述 TextArea
        ├── 状态 Select
        ├── 计划完成时间 DateTimePicker
        ├── 等待对象/原因（条件显示）
        ├── 处置过程时间线
        ├── [追加过程] 输入 + 按钮
        └── [保存] [删除] 按钮
```

### 5.3 任务清单

| 序号 | 任务 | 细节 |
|------|------|------|
| P3-1 | MainLayout | AppLayout + 应用标题「workOrder」 |
| P3-2 | WorkOrderListView 骨架 | Grid 绑定 Service 数据 |
| P3-3 | 新建功能 | Toolbar 按钮 → 打开空 Dialog → 保存 |
| P3-4 | 编辑/详情 Dialog | 点击 Grid 行打开，表单绑定实体 |
| P3-5 | 状态切换 | Select 组件，切换时更新 Service |
| P3-6 | 待回复条件字段 | 状态=WAITING_REPLY 时显示 waitingFor / waitingReason |
| P3-7 | 处置过程时间线 | Dialog 内 VerticalLayout 展示 ProgressLog 列表 |
| P3-8 | 追加过程 | TextField + Button → ProgressLogService.addLog |
| P3-9 | 状态筛选 | MultiSelectComboBox 变更 → 刷新 Grid 数据 |
| P3-10 | 隐藏已完成 | Checkbox 切换 → 过滤 COMPLETED 状态 |
| P3-11 | 拖拽排序 | Grid setRowsDraggable + dropListener → updatePriorities |
| P3-12 | 过期高亮 | setClassNameGenerator：dueDate < now && status != COMPLETED → "overdue-row" |
| P3-13 | 删除功能 | Dialog 内删除按钮 + 确认 |
| P3-14 | 主题样式 | `overdue-row` CSS：红色文字或背景色 |

### 5.4 交互流程

```mermaid
sequenceDiagram
    participant User
    participant ListView
    participant Dialog
    participant Service
    participant DB

    User->>ListView: 打开应用
    ListView->>Service: findByStatuses(filter)
    Service->>DB: SELECT
    DB-->>ListView: 任务列表

    User->>ListView: 拖拽排序
    ListView->>Service: updatePriorities(orderedIds)
    Service->>DB: UPDATE priority

    User->>ListView: 点击任务
    ListView->>Dialog: 打开详情
    User->>Dialog: 追加过程 / 改状态 / 设 deadline
    Dialog->>Service: update / addLog
    Service->>DB: UPDATE / INSERT
    Dialog-->>ListView: 刷新列表
```

### 5.5 验收

- [ ] 全部功能需求（F-001 ~ F-022）可操作
- [ ] 拖拽排序重启后保持
- [ ] 过期任务有明显视觉区分
- [ ] 筛选、隐藏已完成正常工作

---

## 6. 阶段四：桌面打包（P4）

### 6.1 目标

将应用打包为 Windows 可执行程序，双击启动后自动打开工作窗口。

### 6.2 任务清单

| 序号 | 任务 | 细节 |
|------|------|------|
| P4-1 | 创建 DesktopLauncher | 独立 Main 类：启动 Spring Boot → 等待端口就绪 → 打开浏览器 app 模式 |
| P4-2 | 端口检测 | 读取 `server.port`（默认 8080），轮询直到服务可用 |
| P4-3 | 浏览器启动 | Windows：`cmd /c start chrome --app=http://localhost:{port}`；fallback 到 msedge 或默认浏览器 |
| P4-4 | 配置 jpackage | Maven plugin 或脚本：`jpackage --input target --main-jar workorder.jar --main-class DesktopLauncher` |
| P4-5 | 应用图标 | 准备 `.ico` 文件供 jpackage 使用 |
| P4-6 | 数据目录初始化 | 首次启动自动创建 `%USERPROFILE%/.workOrder/data/` |
| P4-7 | 打包验证 | 在干净 Windows 环境双击 exe 测试完整流程 |

### 6.3 DesktopLauncher 伪代码

```java
public class DesktopLauncher {
    public static void main(String[] args) {
        // 1. 后台线程启动 Spring Boot
        ConfigurableApplicationContext ctx =
            SpringApplication.run(WorkOrderApplication.class, args);
        int port = ctx.getEnvironment().getProperty("server.port", Integer.class, 8080);

        // 2. 等待服务就绪
        waitForPort(port);

        // 3. 以 app 模式打开浏览器
        openBrowserAppMode("http://localhost:" + port);
    }
}
```

### 6.4 验收

- [ ] 双击 exe 后 5 秒内出现工作窗口
- [ ] 关闭浏览器窗口后进程退出（或提供托盘最小化，MVP 可不做）
- [ ] 重启后数据仍在（SQLite 持久化）
- [ ] 无 Chrome 时 fallback 方案可用

---

## 7. 测试计划

| 类型 | 范围 | 方式 |
|------|------|------|
| 单元测试 | Service 层 CRUD、筛选、排序、过期判断 | JUnit 5 + H2 或 SQLite 内存模式 |
| 集成测试 | Repository + SQLite 文件模式 | `@DataJpaTest` |
| 手动测试 | UI 全部交互 | 按需求文档验收标准逐项检查 |
| 打包测试 | exe 启动与数据持久化 | 干净 Windows 环境 |

---

## 8. 里程碑与时间建议

| 里程碑 | 内容 | 建议耗时 |
|--------|------|----------|
| M1 | P1 完成，空壳可运行 | 0.5 天 |
| M2 | P2 完成，Service 测试通过 | 1 天 |
| M3 | P3 完成，MVP 功能齐全 | 2-3 天 |
| M4 | P4 完成，exe 可分发 | 0.5-1 天 |
| **合计** | **MVP 交付** | **约 4-5 天** |

> 以上为个人开发节奏估算，实际按进度调整。

---

## 9. 实现顺序依赖

```
P1 脚手架
 └─► P2 领域层（依赖 P1 的数据库配置）
      └─► P3 UI（依赖 P2 的 Service）
           └─► P4 打包（依赖 P3 完整功能）
```

**建议**：P2 完成后先写 Service 单元测试再进入 P3，避免 UI 层调试时混淆业务 bug。

---

## 10. 下一步行动

1. 确认本文档无误
2. 执行 **P1：项目脚手架** — 初始化 Maven 工程并验证启动
3. 按阶段顺序推进至 M4（v1.0 发布）
4. 后续版本排期参见 [版本规划.md](./版本规划.md)
